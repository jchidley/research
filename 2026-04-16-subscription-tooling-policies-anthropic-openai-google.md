...
> **Document role:** policy/entitlement source of truth for subscription-tooling boundaries: what is acceptable through Anthropic/OpenAI/Google/GitHub subscription products, when API/commercial auth is required, and how `pi` + tmux + official CLIs fit. For the current operational routing and cost strategy — including DeepSeek economics, Chinese model alternatives, and Codex reasoning-effort policy — see [`2026-06-14-pi-coding-agent-model-routing-and-cost-strategy.md`](2026-06-14-pi-coding-agent-model-routing-and-cost-strategy.md).
>
